...
Convert a data frame into a frictionless tabular-data-resource (i.e., `fr_tdr` object) with `as_fr_tdr()`.  An `fr_tdr` object is essentially a list of `fr_field`s with table-specific metadata descriptors. Here, we create some metadata based on `?mtcars`:
...
Add a `name` metadata descriptor for each field in the `fr_tdr` object by using the `@` accessor
