...
test_that("fr_date works", {
  date_strings <- c("2023-01-01", "2023-03-01", "2022-11-24")
  fr_date(date_strings, name = "event_date") |>
    vctrs::vec_data() |>
    as.Date() |>
...
test_that("fr_date parsing works", {
  c("2/5/23", "2/5/21", "2/5/18") |>
    fr_date(name = "event_date", parse = TRUE) |>
    vctrs::vec_data() |>
    as.Date() |>
    expect_equal(as.Date(c("2023-02-05", "2021-02-05", "2018-02-05")))
})

test_that("fr_boolean works", {
  c(TRUE, FALSE, TRUE) |>
    fr_boolean(name = "case") |>
    vctrs::vec_data() |>
    expect_equal(c(TRUE, FALSE, TRUE))
})

test_that("fr_boolean parsing works", {
  c("true", "False", "TRUE", " ") |>
    fr_boolean(name = "case", parse = TRUE) |>
    vctrs::vec_data() |>
